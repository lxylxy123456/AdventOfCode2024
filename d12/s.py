...
def solve_2_recu(rec, cont):
	...
	if rec[0] == '.':
		...
		if ((sum(map('?#'.__contains__, rec[:n])) == n) and
			...
			# rec[n+1:].count('#'), rec[n+1:].count('?'), nc - n
			return solve_2_recu(rec[n+1:], cont[1:])
		else:
			return 0
	else:
		s = 0
...
def solve_2(rec, cont):
	# rec.count('#'), rec.count('?'), sum(cont)
	return solve_2_recu(rec, tuple(cont))
